...
def turn_on(device_name: str):
    config = json.loads(ssm_service.get_ssm_parameter(os.environ["SECRET_PARAM"]) )
    print(config)
    device = next((stat for stat in devices if stat["name"] == device_name), None)
    API_ENDPOINT = "http://{}/apps/api/293/devices/{}/on?access_token={}".format(config.get("host"), device.get("id") ,config.get("token") )
    ...
    set_level(device.get("id"), "75", config)
...
def turn_off(device_name: str):
    ...
    if device.get("off_color", None) != None:
        ...
        API_ENDPOINT = "http://{}/apps/api/293/devices/{}/off?access_token={}".format(config.get("host"), device.get("id") ,config.get("token") )
        response = requests.get(url = API_ENDPOINT)
        print("RESPONSE STATUS OFF: %s"%response.status_code) 

def set_level(device_id: str, level_val: str, config: dict):
    API_ENDPOINT = "http://{}/apps/api/293/devices/{}/setLevel/{}?access_token={}".format(config.get("host"), device_id , level_val, config.get("token") )
    response = requests.get(url = API_ENDPOINT)
    print("RESPONSE STATUS LEVEL: %s"%response.status_code)

def set_color(device_id: str, color_val: str, config: dict):
    API_ENDPOINT = "http://{}/apps/api/293/devices/{}/{}?access_token={}".format(config.get("host"), device_id , color_val, config.get("token") )
    response = requests.get(url = API_ENDPOINT)
